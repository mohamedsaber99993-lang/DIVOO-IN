...
// دالة معالجة العمليات غير المتزامنة خطوة بخطوة (Async Generator Step)
...
// محاكي الـ Async/Await البرمجي للبيئات القديمة
...
function _classPrivateMethodGet(receiver, privateSet, fn) {
  if (!privateSet.has(receiver)) {
    throw new TypeError("محاولة استدعاء دالة خاصة (Private Method) على كائن غير برمجى مستقل");
  }
  return fn;
}

// تحويل دوال نظام الملفات الإدخال والإخراج الكلاسيكية إلى وعود (Promises)
const fs = {
...
// الثوابت الأساسية المحددة لنوع ضغط الملصق (WebP Compression Types)
const constants = {
  TYPE_LOSSY: 0,    // ضغط مع فقدان بعض الجودة (مثل الصور الفوتوغرافية العادية)
  TYPE_LOSSLESS: 1, // ضغط بدون أي فقدان في الجودة (شفافية كاملة جودة عالية)
  TYPE_EXTENDED: 2  // نظام ممتد يدعم الرسوم المتحركة وحقن الميتاداتا والألوان الإضافية
};
exports.constants = constants;

/* =========================================================================
   عمليات معالجة البايتات وقراءة أبعاد الصورة رياضياً من البافر (Bitwise Operations)
   ========================================================================= */

// جلب عرض الصورة المضغوطة برمجياً بنظام الـ Lossy عبر إزاحة البايتات
function VP8Width(data) {
  let n = (data[7] << 8) | data[6];
  return n & 0b0011111111111111;
}

// جلب طول الصورة المضغوطة برمجياً بنظام الـ Lossy
function VP8Height(data) {
  let n = (data[9] << 8) | data[8];
  return n & 0b0011111111111111;
}

// جلب عرض الصورة عالية الجودة Lossless (بايتات الـ VP8L تبدأ من الإطار الثاني)
function VP8LWidth(data) {
  let n = (data[2] << 8) | data[1];
  return (n & 0b0011111111111111) + 1;
}

// جلب طول الصورة عالية الجودة Lossless عبر الإزاحة الثنائية بمقدار 6 بت
...
// الفحص الثنائي لمعرفة ما إذا كان إطار الـ Lossless يحتوي على قناة الشفافية (Alpha Channel)
function doesVP8LHaveAlpha(data) {
  return !!(data[4] & 0b00010000);
}

// دالة توليد الأجزاء الهيكلية الأساسية لملف الـ WebP (Chunk Creator)
function createBasicChunk(name, data) {
  let chunk = Buffer.alloc(8),
    size = data.length,
    out;
  chunk.write(name, 0);
  chunk.writeUInt32LE(size, 4); // كتابة الحجم بنظام Little-Endian
  out = [chunk, data];

  // إذا كان الحجم فردياً، يتم إضافة بايت فارغ (Null Byte) لتوحيد مصفوفة البايتات الزوجية
  if (size & 1) {
    out[2] = nullByte;
  }
...
// تسجيل المجموعات الخاصة (WeakSets) لحماية وتأمين العمليات الداخلية للكلاس
...
/* =========================================================================
   الكلاس الرئيسي لإدارة هندسة وهيكل صور وملصقات الـ WebP
   ========================================================================= */
class Image {
  constructor() {
    // ربط كافة العمليات الحيوية والخاصة بالبنية الأساسية للكائن
    _read.add(this);
  ...
  // تصفير وتهيئة الكائن للإعدادات الافتراضية
  clear() {
    this.data = null;
    this.path = "";
    this.loaded = false;
  ...
  // جلب عرض الملصق بناءً على نوع الخوارزمية المستخدمة داخله
  get width() {
  ...
  // التحقق مما إذا كان الملصق متحركاً (يحتوي على خط زمني وإطارات متعددة)
  get hasAnim() {
  ...
  /* ميثودز إدارة عينات الألوان والملفات التعريفية المدمجة */
  get iccp() {
  ...
  /* ميثودز جلب وحقن حقوق الملصق (EXIF Data) وهي المسؤولة عن إظهار اسم الحزمة والمصنع في واتساب */
  get exif() {
  ...
  // دالة تحميل وقراءة ملف ميديا WebP من القرص الصلب
  load(path) {
    var _this = this;
  ...
  // دالة تفكيك الملصق المتحرك واستخراج إطاراته كملصقات منفصلة (Demuxing)
  demuxAnim(path, frame = -1, prefix = "#FNAME#") {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (end == 0) {
        throw new Error("ملف الـ WebP هذا ليس ملفاً متحركاً أو لا يحتوي على إطارات متعددة");
      }
      if (frame != -1) {
        start = end = frame;
      }
  ...
  // دالة استبدال إطار معين داخل الملصق المتحرك بإطار صورة جديد
  replaceFrame(path, frame) {
    var _this3 = this;
    return _asyncToGenerator(function* () {
      if (!_this3.hasAnim) {
        throw new Error("الصورة أو الملصق ليس متحركاً لتعديل إطاراته");
      }
      if (frame < 0 || frame >= _this3.frameCount) {
        throw new Error(`مؤشر الإطار الممرر [${frame}] خارج الحدود المسموح بها لتعديل هذا الملصق`);
      }
      ...
      switch (r.type) {
        ...
        case constants.TYPE_EXTENDED:
          if (r.hasAnim) {
            throw new Error("دمج ومزج ملصقين متحركين معاً غير مدعوم حالياً في هذه النسخة");
          }
          break;
        default:
          throw new Error("نوع ميديا الـ WebP الممررة غير معروف");
      }

      // تنظيف وهيكلة الإطارات القديمة لإحلال البايتات الجديدة
      switch (_this3.anim.frames[frame].type) {
        ...
        case constants.TYPE_LOSSLESS:
          ...
          break;
        default:
          throw new Error("نوع الإطار الداخلي المراد حذفه غير مدعوم");
      }
  ...
  // الدالة الإستاتيكية الضخمة المسؤولة عن تركيب وصناعة الملصقات المتحركة بالكامل (Muxing)
  static muxAnim({
    path,
    frames,
    width = 0,
    ...
    return _asyncToGenerator(function* () {
      ...
      if (frames.length == 0) {
        throw new Error("لا توجد أي إطارات صور ممررة لتركيب الملصق المتحرك منها");
      } else if (_width <= 0 || _width > 1 << 24) {
        throw new Error("العرض المحدد للملصق يقع خارج النطاق الثنائي المسموح به");
      } else if (_height <= 0 || _height > 1 << 24) {
        throw new Error("الارتفاع المحدد للملصق يقع خارج النطاق الثنائي المسموح به");
      } else if (_height * _width > Math.pow(2, 32) - 1) {
        throw new Error(`حجم أبعاد مساحة الملصق ضخم جداً ومتجاوز للحد الأقصى`);
      }

      // كتابة ترويسة ملف الـ WebP القياسية ثنائياً بنظام الـ RIFF контейнер
      header.write("RIFF", 0);
      header.write("WEBP", 8);
      chunk.write("VP8X", 0);
      ...
      // إعداد بيانات خط الرسوم المتحركة الافتراضي وتحديد عدد مرات التكرار (Loops)
      ...
      // حلقة تكرارية لمعالجة وحقن بايتات كل إطار (ANMF Chunks) في الملف المخرج
      for (let i = 0, l = frames.length; i < l; i++) {
        ...
        switch (img.data.type) {
          ...
          default:
            throw new Error("تم العثور على نوع صورة غير معروف للدمج والمزج");
        }
      ...
      // فتح وكتابة الملف النهائي الناتج على مسار الحفظ المحدد للبوت
...
/* =========================================================================
   الدوال الفرعية والعمليات الخلفية الخاصة بالتحليل الثنائي للملفات (Parsers)
   ========================================================================= */

var _convertToExtended2 = function _convertToExtended2() {
  if (!this.loaded) {
    throw new Error("لم يتم تحميل أي صورة مسبقاً في الذاكرة لتوسيع بايتها");
  }
...
var _demuxFrame2 = (function () {
  var _demuxFrame3 = _asyncToGenerator(function* (path, frame) {
    ...
    if (this.data.extended.hasICC || this.data.extended.hasEXIF || this.data.extended.hasXMP || (frame.vp8 && frame.vp8.alpha)) {
      ...
      if ((frame.vp8l && frame.vp8l.alpha) || (frame.vp8 && frame.vp8.alpha)) chunk[8] |= 0b00010000;
    ...
    if (frame.vp8l) {
      ...
      throw new Error("خطأ هيكلي: لا توجد بيانات ضغط صالحة داخل هذا الإطار المستخرج");
    }
  ...
  return function _demuxFrame2(_x2, _x3) {
...
var _readHeader2 = (function () {
  var _readHeader3 = _asyncToGenerator(function* (fd) {
    ...
    if (bytesRead != 12) throw new Error("تم الوصول لنهاية الملف بشكل مفاجئ أثناء محاولة قراءة الترويسة");
    if (buf.toString("utf8", 0, 4) != "RIFF") throw new Error("صيغة الملف غير صالحة (لا يحتوي على ترويسة RIFF الفوقية)");
    if (buf.toString("utf8", 8, 12) != "WEBP") throw new Error("الملف الممرر ليس ملف ميديا WebP رسمي");

    return { fileSize: buf.readUInt32LE(4) };
  });
  return function _readHeader2(_x4) { return _readHeader3.apply(this, arguments); };
})();
...
var _readChunkHeader2 = (function () {
  var _readChunkHeader3 = _asyncToGenerator(function* (fd) {
    ...
    else if (bytesRead < 8) throw new Error("الملف تالف أو ناقص أثناء معالجة رأس الـ Chunk");
  ...
  return function _readChunkHeader2(_x5) { return _readChunkHeader3.apply(this, arguments); };
})();
...
var _readChunk_raw2 = (function () {
  var _readChunk_raw3 = _asyncToGenerator(function* (n, fd, size) {
    ...
    if (bytesRead != size) throw new Error(`فشل قراءة كتلة البيانات من نوع ${n}`);
    if (size & 1) yield fs.read(fd, discard, 0, 1, undefined);

    return { raw: buf };
  });
  return function _readChunk_raw2(_x6, _x7, _x8) { return _readChunk_raw3.apply(this, arguments); };
})();
...
var _readChunk_VP2 = (function () {
  var _readChunk_VP3 = _asyncToGenerator(function* (fd, size) {
    ...
    if (bytesRead != size) throw new Error("انقطاع قراءة البايتات أثناء فحص كتلة الـ VP8");
    if (size & 1) yield fs.read(fd, discard, 0, 1, undefined);
  ...
  return function _readChunk_VP2(_x9, _x10) { return _readChunk_VP3.apply(this, arguments); };
})();

var _readChunk_VP8_buf2 = function _readChunk_VP8_buf2(buf, size, cursor) {
  if (cursor >= buf.length) throw new Error("تجاوز بافر الذاكرة المحدود عند محاولة قراءة أبعاد الـ VP8");
...
var _readChunk_VP8L2 = (function () {
  var _readChunk_VP8L3 = _asyncToGenerator(function* (fd, size) {
    ...
    if (bytesRead != size) throw new Error("انقطاع تدفق بايتات الـ Lossless VP8L");
    if (size & 1) yield fs.read(fd, discard, 0, 1, undefined);
  ...
  return function _readChunk_VP8L2(_x11, _x12) { return _readChunk_VP8L3.apply(this, arguments); };
})();

var _readChunk_VP8L_buf2 = function _readChunk_VP8L_buf2(buf, size, cursor) {
  if (cursor >= buf.length) throw new Error("تجاوز بافر الذاكرة عند معالجة كتلة الـ VP8L");
  let raw = buf.slice(cursor, cursor + size);
  return {
    raw,
...
var _readChunk_VP8X2 = (function () {
  var _readChunk_VP8X3 = _asyncToGenerator(function* (fd, size) {
    ...
    if (bytesRead != size) throw new Error("حدث خطأ أثناء فك وتأويل كتلة الـ VP8X الممتدة");
  ...
  return function _readChunk_VP8X2(_x13, _x14) { return _readChunk_VP8X3.apply(this, arguments); };
})();
...
var _readChunk_ANIM2 = (function () {
  var _readChunk_ANIM3 = _asyncToGenerator(function* (fd, size) {
    ...
    if (bytesRead != size) throw new Error("فشل قراءة بايتات الأنيميشن التكوينية ANIM");
  ...
  return function _readChunk_ANIM2(_x15, _x16) { return _readChunk_ANIM3.apply(this, arguments); };
})();
...
var _readChunk_ANMF2 = (function () {
  var _readChunk_ANMF3 = _asyncToGenerator(function* (fd, size) {
    ...
    if (bytesRead != size) throw new Error("فشل قراءة الإطار الفرعي المتحرك ANMF من الملف");
    if (size & 1) yield fs.read(fd, discard, 0, 1, undefined);
  ...
  return function _readChunk_ANMF2(_x17, _x18) { return _readChunk_ANMF3.apply(this, arguments); };
})();
...
var _readChunk_ALPH2 = (function () {
  ...
  return function _readChunk_ALPH2(_x19, _x20) { return _readChunk_ALPH3.apply(this, arguments); };
})();
...
var _readChunk_ICCP2 = (function () {
  ...
  return function _readChunk_ICCP2(_x21, _x22) { return _readChunk_ICCP3.apply(this, arguments); };
})();
...
var _readChunk_EXIF2 = (function () {
  ...
  return function _readChunk_EXIF2(_x23, _x24) { return _readChunk_EXIF3.apply(this, arguments); };
})();
...
var _readChunk_XMP2 = (function () {
  ...
  return function _readChunk_XMP2(_x25, _x26) { return _readChunk_XMP3.apply(this, arguments); };
})();
...
var _readChunk_Skip2 = (function () {
  var _readChunk_Skip3 = _asyncToGenerator(function* (fd, size) {
    let buf = Buffer.alloc(size), discard = Buffer.alloc(1);
    let { bytesRead } = yield fs.read(fd, buf, 0, size, undefined);
    if (bytesRead != size) throw new Error("حدث خطأ ما أثناء محاولة تخطي وتجاوز قراءة كتلة تالفة");
    if (size & 1) yield fs.read(fd, discard, 0, 1, undefined);
  });
  return function _readChunk_Skip2(_x27, _x28) { return _readChunk_Skip3.apply(this, arguments); };
})();

// الدالة الأساسية لتحليل البنية الهيكلية الكاملة وتفسير محتوى ملف الـ WebP (The Big Parser Loop)
var _read2 = (function () {
  var _read3 = _asyncToGenerator(function* (path) {
    ...
    while (keepLooping) {
      ...
      // الفحص البرمجي للوسوم الثنائية الشهيرة في ملفات الميديا لتفكيكها بدقة داخل البوت
      switch (fourCC) {
        case "VP8 ":
          if (!out.vp8) out.vp8 = yield _classPrivateMethodGet(this, _readChunk_VP, _readChunk_VP2).call(this, fd, size);
          else yield _classPrivateMethodGet(this, _readChunk_Skip, _readChunk_Skip2).call(this, fd, size);
          if (first) { out.type = constants.TYPE_LOSSY; keepLooping = false; }
          break;

        case "VP8L":
          if (!out.vp8l) out.vp8l = yield _classPrivateMethodGet(this, _readChunk_VP8L, _readChunk_VP8L2).call(this, fd, size);
          else yield _classPrivateMethodGet(this, _readChunk_Skip, _readChunk_Skip2).call(this, fd, size);
        ...
        case "ALPH":
          if (!out.alph) out.alph = yield _classPrivateMethodGet(this, _readChunk_ALPH, _readChunk_ALPH2).call(this, fd, size);
          else yield _classPrivateMethodGet(this, _readChunk_Skip, _readChunk_Skip2).call(this, fd, size);
          break;

        case "ICCP":
          if (!out.iccp) out.iccp = yield _classPrivateMethodGet(this, _readChunk_ICCP, _readChunk_ICCP2).call(this, fd, size);
          else yield _classPrivateMethodGet(this, _readChunk_Skip, _readChunk_Skip2).call(this, fd, size);
          break;

        case "EXIF":
          if (!out.exif) out.exif = yield _classPrivateMethodGet(this, _readChunk_EXIF, _readChunk_EXIF2).call(this, fd, size);
          else yield _classPrivateMethodGet(this, _readChunk_Skip, _readChunk_Skip2).call(this, fd, size);
          break;

        case "XMP ":
          if (!out.xmp) out.xmp = yield _classPrivateMethodGet(this, _readChunk_XMP, _readChunk_XMP2).call(this, fd, size);
          else yield _classPrivateMethodGet(this, _readChunk_Skip, _readChunk_Skip2).call(this, fd, size);
          break;
  ...
  return function _read2(_x29) { return _read3.apply(this, arguments); };
})();
